...
import synthetic.prog_delta as prog
...
genvars_names = {GenVar.ORIGID: 'orig',
                 ...
                 GenVar.EDGES: 'edge_ratio'} # $edges has the e/E ratio at the current time
...
def create_exo_generator(directed, init_random=False):
    ...
    if directed:
        genvars = (GenVar.ORIGID, GenVar.TARGID, GenVar.ORIGINDEG,
                   ...
                   GenVar.DIST, GenVar.DIRDIST, GenVar.REVDIST,GenVar.EDGES)
    else:
        genvars = (GenVar.ORIGID, GenVar.TARGID, GenVar.ORIGDEG,
                   GenVar.TARGDEG, GenVar.DIST,GenVar.EDGES)
...
def create_endo_generator(directed, init_random=False):
    if directed:
        genvars = (GenVar.ORIGINDEG, GenVar.ORIGOUTDEG, GenVar.TARGINDEG,
                   ...
                   GenVar.REVDIST,GenVar.EDGES)
    else:
        genvars = (GenVar.ORIGDEG, GenVar.TARGDEG, GenVar.DIST,GenVar.EDGES)
    return Generator(genvars, directed, init_random=init_random)
...
class Generator(object):
    ...
    def distance(self, orig, targ, mode, max_dist=DEFAULT_MAX_DIST):
        ...
        sp = self.net.get_shortest_paths(orig, to=targ, mode=mode)[0]        
        if len(sp) != 0:
            return min(max_dist, len(sp) - 1) #! This was max..corrected to min
        else:
            return max_dist + 1
    ...
    def set_prog_vars(self, orig, targ):
        ...
        for i in range(self.var_count):
            ...
            #    GenVar.ORIGID: orig,
            #    GenVar.TARGID: targ,
            #    GenVar.ORIGDEG: self.net.degree(orig, mode=ALL),
            #    GenVar.ORIGINDEG: self.net.degree(orig, mode=IN),
            #    GenVar.ORIGOUTDEG: self.net.degree(orig, mode=OUT),
            #    GenVar.TARGDEG: self.net.degree(targ, mode=ALL),
            #    GenVar.TARGINDEG: self.net.degree(targ, mode=IN),
            #    GenVar.TARGOUTDEG: self.net.degree(targ, mode=OUT),
            #    GenVar.DIST: self.distance(orig, targ, mode=ALL),
            #    GenVar.DIRDIST: self.distance(orig, targ, mode=OUT),
            #    GenVar.REVDIST: self.distance(orig, targ, mode=IN),
            #    GenVar.EDGES:  self.net.ecount()/self.edges

            # }
            
            # self.prog.vars[i] = conditions[gv]
            ...
            elif gv==GenVar.EDGES:
                self.prog.vars[i] = self.net.ecount()/self.edges
    ...
    def cycle(self, master=None):
        ...
        total_weight = 0.
        warnings.filterwarnings("ignore") #! REMOVE if error
        for i in range(self.trials):
